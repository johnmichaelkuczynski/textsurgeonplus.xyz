...
- **Authentication**: Direct Google OAuth ONLY (passport-google-oauth20). No Clerk, no username login, no auth middlemen. Server (`server/auth.ts`): `GET /api/auth/google` → 302 to Google (scope openid email profile, `prompt=select_account`, `state:true` CSRF) → `GET /api/auth/google/callback` upserts user + passport session → redirect `/`. Creds read from `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` (fallback `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`); prod callback host from `REPLIT_DOMAINS[0]` fallback textsurgeonplus.xyz. `/api/auth/me` (401 anon), `POST /api/auth/logout` destroys session. Admin (`/api/admin/visits`) restricted to johnmichaelkuczynski@gmail.com. Identity middleware in `server/routes.ts` forces `username` in body/query from the session and strips it for anonymous requests — clients can never act as another user. Client: signed-out visitors see a landing page with a single "Sign in with Google" link (`<a href="/api/auth/google" target="_top">` to escape the preview iframe). SESSION_SECRET required in production (fail-fast). Google Cloud Console OAuth client must list both callback URLs (dev riker.replit.dev + textsurgeonplus.xyz).
