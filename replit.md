...
- **Authentication**: NONE (removed at owner's explicit request, July 2026). No Google OAuth, no Clerk, no username login. The app is fully open — anyone can use all analysis features anonymously. `server/auth.ts` keeps only session middleware + passport init (so `req.isAuthenticated()` exists, always false), plus inert `/api/auth/user`, `/api/auth/me`, logout, and admin endpoints (admin always 403 since no one can authenticate). Identity middleware in `server/routes.ts` still strips client-supplied `username`, so per-user data endpoints (history, stylometrics saves) are inert. Client has no login UI, no landing page, no paywall (`hasCredits = true`). DO NOT re-add any login system without the owner's explicit instruction.
