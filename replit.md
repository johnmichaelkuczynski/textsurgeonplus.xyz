...
- **Authentication**: OPTIONAL Google login (reinstated at owner's request, July 2026 — reversing the earlier full removal). The app remains fully open: all analysis/TTS features work anonymously with no gates, no landing page, no paywall (`hasCredits = true` client-side). Signing in with Google only enables history, saved profiles, and admin. `server/auth.ts` uses passport-google-oauth20; credentials read from `GOOGLE_LOGIN_CLIENT_ID/SECRET` (preferred, reusable across owner's apps) with fallback to `GOOGLE_OAUTH_*` then `GOOGLE_*`; if absent, login silently disables and the app still runs. Callback path is `/auth/google/callback` (NO `/api` prefix — must match the redirect URIs the owner registers in Google Cloud Console; legacy `/api/auth/google/callback` alias also handled). `GET /api/auth/google` starts login. In the Replit preview iframe the sign-in button opens a new tab (Google blocks framed sign-in). Identity middleware in `server/routes.ts` still strips client-supplied `username` for anonymous requests (anti-spoofing — must stay). Logged-in users get credit checks/deduction on analysis endpoints (charged exactly once per request).
